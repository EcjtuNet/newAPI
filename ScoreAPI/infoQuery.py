...
def getScoreInfo(year,term):
    result = loginRequest.readUrl('http://jwxt.ecjtu.jx.cn/scoreQuery/stuScoreQue_getStuScore.action')
...
def getClassInfo(year,term):
    ...
    result = loginRequest.readUrl('http://jwxt.ecjtu.jx.cn/Schedule/Schedule_getUserSchedume.action')
    soup = BeautifulSoup(result,"html.parser")
    #获取包含课程信息的table内容
